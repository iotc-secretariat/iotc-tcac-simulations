...
COL_RC_FLEET = color_table(unique_colors(length(unique(RC$ENTITY_CODE))))
COL_RC_FLEET[, FILL_BY := sort(unique(RC$ENTITY_CODE))]
COL_RC_FLEET[, FILL_BY := factor(FILL_BY, levels = sort(unique(RC$ENTITY_CODE)))]

### All species ####
RC_FLEET_BARPLOT = 
  value_bar(data = RC, value = "CATCH_MT", time = "YEAR", fill_by = "ENTITY_CODE", colors = COL_RC_FLEET, x_axis_label = "", y_axis_label = "Total catch (x1,000 t)", scale = 1000, num_legend_rows = 3) + theme(legend.position = "bottom")

save_plot("./outputs/charts/RC_FLEET_BARPLOT.png", RC_FLEET_BARPLOT, 8, 4.5)

### Albacore ####
RC_ALB_FLEET_COLORS = COL_RC_FLEET[FILL_BY %in% RC[SPECIES_CODE == "ALB", ENTITY_CODE]]

RC_ALB_FLEET_BARPLOT = 
  value_bar(data = RC[SPECIES_CODE == "ALB"], value = "CATCH_MT", time = "YEAR", fill_by = "ENTITY_CODE", colors = RC_ALB_FLEET_COLORS, x_axis_label = "", y_axis_label = "Total catch (x1,000 t)", scale = 1000, num_legend_rows = 3) + theme(legend.position = "bottom")

save_plot("./outputs/charts/RC_ALB_FLEET_BARPLOT.png", RC_ALB_FLEET_BARPLOT, 8, 4.5)

### Bigeye tuna ####
RC_BET_FLEET_COLORS = color_table(unique_colors(length(unique(RC[SPECIES_CODE == "BET", ENTITY_CODE]))))

RC_BET_FLEET_BARPLOT = 
  value_bar(data = RC[SPECIES_CODE == "BET"], value = "CATCH_MT", time = "YEAR", fill_by = "ENTITY_CODE", colors = RC_BET_FLEET_COLORS, x_axis_label = "", y_axis_label = "Total catch (x1,000 t)", scale = 1000, num_legend_rows = 3) + theme(legend.position = "bottom")

save_plot("./outputs/charts/RC_BET_FLEET_BARPLOT.png", RC_BET_FLEET_BARPLOT, 8, 4.5)

### Skipjack tuna ####
RC_SKJ_FLEET_COLORS = color_table(unique_colors(length(unique(RC[SPECIES_CODE == "SKJ", ENTITY_CODE]))))

RC_SKJ_FLEET_BARPLOT = 
  value_bar(data = RC[SPECIES_CODE == "SKJ"], value = "CATCH_MT", time = "YEAR", fill_by = "ENTITY_CODE", colors = RC_SKJ_FLEET_COLORS, x_axis_label = "", y_axis_label = "Total catch (x1,000 t)", scale = 1000, num_legend_rows = 3) + theme(legend.position = "bottom")

save_plot("./outputs/charts/RC_SKJ_FLEET_BARPLOT.png", RC_SKJ_FLEET_BARPLOT, 8, 4.5)

### Swordfish ####
RC_SWO_FLEET_COLORS = color_table(unique_colors(length(unique(RC[SPECIES_CODE == "SWO", ENTITY_CODE]))))

RC_SWO_FLEET_BARPLOT = 
  value_bar(data = RC[SPECIES_CODE == "SWO"], value = "CATCH_MT", time = "YEAR", fill_by = "ENTITY_CODE", colors = RC_SWO_FLEET_COLORS, x_axis_label = "", y_axis_label = "Total catch (x1,000 t)", scale = 1000, num_legend_rows = 3) + theme(legend.position = "bottom")

save_plot("./outputs/charts/RC_SWO_FLEET_BARPLOT.png", RC_SWO_FLEET_BARPLOT, 8, 4.5)

### Yellowfin tuna ####
RC_YFT_FLEET_COLORS = color_table(unique_colors(length(unique(RC[SPECIES_CODE == "YFT", ENTITY_CODE]))))

RC_YFT_FLEET_BARPLOT = 
  value_bar(data = RC[SPECIES_CODE == "YFT"], value = "CATCH_MT", time = "YEAR", fill_by = "ENTITY_CODE", colors = RC_YFT_FLEET_COLORS, x_axis_label = "", y_axis_label = "Total catch (x1,000 t)", scale = 1000, num_legend_rows = 3) + theme(legend.position = "bottom")
...
RC_NJA[ENTITY_CODE == j, ORIGIN := "National"]
RC_NJA[ENTITY_CODE != j, ORIGIN := "Foreign"]
